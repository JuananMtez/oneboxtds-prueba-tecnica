package prueba.tecnica.oneboxtds.infrastructure.rest.cart.controller;
...
import prueba.tecnica.oneboxtds.infrastructure.rest.cart.CartController;
import prueba.tecnica.oneboxtds.infrastructure.rest.cart.mapper.ICartRestMapper;
import prueba.tecnica.oneboxtds.infrastructure.rest.cart.message.CartDto;
import prueba.tecnica.oneboxtds.infrastructure.rest.error.message.ErrorDto;
import prueba.tecnica.oneboxtds.infrastructure.rest.product.mapper.IProductRestMapper;
...
public class CartControllerTest {
  ...
  public void addProductToCart_NotFound() {
    ...
    when(productRestMapper.asProductList(productDtoList)).thenReturn(productList);
    when(cartService.addProductsToCart(cartId, productList)).thenThrow(CartNotFoundException.class);
  ...
  public void deleteCartById_NotFound() {
    ...
    doThrow(new CartNotFoundException("Cart not found")).when(cartService).deleteCartById(cartId);
  ...
  public void removeProductsFromCart_UnexpectedError() {
    ...
    when(cartService.removeProductsFromCart(cartId, productIds)).thenThrow(RuntimeException.class);
  ...
  private CartDto createCartDto() {
    return new CartDto(
        UUID.randomUUID(), List.of(createProductDto()), LocalDateTime.now(), LocalDateTime.now());
  }

  private Cart createCart() {
    return new Cart(
        UUID.randomUUID(), List.of(createProduct()), LocalDateTime.now(), LocalDateTime.now());
  }
}
