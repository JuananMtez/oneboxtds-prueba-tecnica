package com.juananmtez.smallecommerce.infrastructure.repository.cart;
...
import com.juananmtez.smallecommerce.infrastructure.repository.cart.mapper.ICartRepositoryMapper;
...
public class CartRepositoryImplTest {
    ...
    void saveCart_ThrowsAlreadyExists() {
        ...
        CartAlreadyExistsException exception =
                assertThrows(
                        CartAlreadyExistsException.class, () -> cartRepositoryImpl.saveCart(cart));
        assertNotNull(exception);
        verify(cartRepositoryMapper, times(1)).asCartEntity(cart);
        verify(database, times(1)).saveCartEntity(cartEntity);
    }
    ...
    void updateCart_ThrowsNotFound() {
        ...
        CartNotFoundException exception =
                assertThrows(
                        CartNotFoundException.class, () -> cartRepositoryImpl.updateCart(cart));
        assertNotNull(exception);
        verify(cartRepositoryMapper, times(1)).asCartEntity(cart);
        verify(database, times(1)).updateCartEntity(cartEntity);
    }
    ...
    void findCartById_ThrowsNotFound() {
        ...
        CartNotFoundException exception =
                assertThrows(
                        CartNotFoundException.class, () -> cartRepositoryImpl.findCartById(id));
        assertNotNull(exception);
        verify(database, times(1)).findCartEntityById(id);
    }
    ...
    void deleteCartById_NotFound() {
        ...
        doThrow(new CartNotFoundException("Cart not found"))
                .when(database)
                .deleteCartEntityById(id);

        CartNotFoundException exception =
                assertThrows(
                        CartNotFoundException.class, () -> cartRepositoryImpl.deleteCartById(id));
        assertNotNull(exception);
        verify(database, times(1)).deleteCartEntityById(id);
    }
    ...
    void findAllUnusedCarts_ThrowsException() {
        ...
        RuntimeException exception =
                assertThrows(
                        ...
                        () -> cartRepositoryImpl.deleteAllUnusedCarts(expiration));
    ...
    private CartEntity createCartEntity() {
        return new CartEntity(
                UUID.randomUUID(), List.of(), LocalDateTime.now(), LocalDateTime.now());
    }
