...
public class CartRepositoryMapperImplTest {
    ...
    void given_CartEntity_convert_to_Cart() {
        ...
        for (int i = 0; i < response.getProducts().size(); i++) {
            assertEquals(
                    response.getProducts().get(i).getId(), cartEntity.getProducts().get(i).getId());
            assertEquals(
                    response.getProducts().get(i).getAmount(),
                    cartEntity.getProducts().get(i).getAmount());
            assertEquals(
    ...
    void given_CartEntityList_convert_to_CartList() {
        ...
        for (int i = 0; i < response.size(); i++) {
            ...
            assertEquals(
                    response.get(i).getCreationDate(), cartEntityList.get(i).getCreationDate());
            assertEquals(
                    response.get(i).getModificationDate(),
                    cartEntityList.get(i).getModificationDate());
