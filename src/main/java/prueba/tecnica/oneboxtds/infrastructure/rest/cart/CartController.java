package prueba.tecnica.oneboxtds.infrastructure.rest.cart;
...
public class CartController {
  ...
  private ICartRestMapper cartRestMapper;
      ...
      return ResponseEntity.status(HttpStatus.CREATED)
          .body(
              cartRestMapper.asCartDto(
                  cartService.createCart(productRestMapper.asProductList(products))));
  ...
  public ResponseEntity<?> addProductsToCart(
      @PathVariable @NotNull UUID id, @RequestBody @NotNull List<@Valid ProductDto> products) {
    ...
    try {
      return ResponseEntity.status(HttpStatus.OK)
          .body(
              cartRestMapper.asCartDto(
                  cartService.addProductsToCart(id, productRestMapper.asProductList(products))));
    } catch (CartNotFoundException e) {
      return ResponseEntity.status(HttpStatus.NOT_FOUND)
          .body(new ErrorDto(e.getMessage(), ErrorEnum.CART_NOT_FOUND));
  ...
  public ResponseEntity<?> removeProductsFromCart(
    ...
    logger.info("{} Delete products with id {} from cart with id {}", LOG_HEADER, productIds, id);

    try {
      return ResponseEntity.status(HttpStatus.OK)
          .body(cartRestMapper.asCartDto(cartService.removeProductsFromCart(id, productIds)));
    } catch (CartNotFoundException e) {
      return ResponseEntity.status(HttpStatus.NOT_FOUND)
          .body(new ErrorDto(e.getMessage(), ErrorEnum.CART_NOT_FOUND));
